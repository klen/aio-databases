...
pytestmark = pytest.mark.skip("trio_asyncio is not supported from trio 0.22.1")
...
async def test_base(user_cls: Model):
    ...
    async with trio_asyncio.open_loop(), Database(
        "triopg://test:test@localhost:5432/tests", convert_params=True
    ) as db:
